...
export class LaneManager {
  ...
  async promptExisting(
    ...
    const drive = this.turnDriver.driveContinuation(run, conn, turnPrompt, correction);
    this.trackDrive(id, drive);
  ...
  private startResumeTurn(
    ...
    run.reopenForResume();
    this.trackDrive(run.id, this.turnDriver.attemptInitialTurn(run, spec, turnPrompt, 1, correction));
