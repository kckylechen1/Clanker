...
- **Model token** (first token if it is a shortname, or a `--model` value): pass it through as `model` **unchanged** — the server resolves shortnames from the single source in `src/constants.ts` (`OC_MODEL_ALIASES`). For reference, the current map is `glm → zhipuai-coding-plan/glm-5.2`, `ds → deepseek/deepseek-v4-pro`, `kimi → kimi-for-coding/k3`, `free → opencode/deepseek-v4-flash-free`, `composer → xai/grok-composer-2.5-fast`, `grok45 → xai/grok-4.5`; a value containing `/` is a full id. If no model token is given, default to `glm`.
