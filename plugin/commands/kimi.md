---
description: "Dispatch a task to Clanker: Kimi via Opencode ACP."
argument-hint: "[--background|--wait] [--write] [--effort <effort>] <task>"
allowed-tools: Agent
---

Dispatch the task below to **Clanker: Kimi** via the `Agent` tool. The fixed model token is `kimi`. Read-only calls use `subagent_type: "clanker:oc"`; write calls use the packaged `subagent_type: "clanker:supervisor"`.
...
- Fixed `model: "kimi"`; do not ask the user for a model and do not infer another one.
- `--write` present -> `read_only: false`, a mandatory `worktree`, and `subagent_type: "clanker:supervisor"`. If the user did not name a branch, generate `clanker/kimi-<short-timestamp>`. Absent `--write` -> `read_only: true` and `subagent_type: "clanker:oc"`.
...
- The task text must be: "Dispatch as Clanker: Kimi. lane=opencode. prompt=<task>. read_only=<bool>. worktree=<branch or omit>. model=kimi. effort=<effort or omit>. Follow your relay/supervisor protocol."
- Do not call MCP dispatch tools in the main conversation.
- Do not wait for the subagent or relay a final result in this turn.
- After launching, tell the user: "Clanker: Kimi started in the Claude Code background task list."
