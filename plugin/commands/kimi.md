...
Dispatch the task below to **Clanker: Kimi** by invoking the `clanker:oc` subagent via the `Agent` tool (`subagent_type: "clanker:oc"`). The fixed model token is `kimi`.
...
- `--write` present -> `read_only: false` and a `worktree` is mandatory. If the user did not name a branch, generate `clanker/kimi-<short-timestamp>`. Absent `--write` -> `read_only: true`.
...
- The task text must be: "Dispatch as Clanker: Kimi. prompt=<task>. read_only=<bool>. worktree=<branch or omit>. model=kimi. effort=<effort or omit>. Follow your relay protocol."
