...
Dispatch the task below to **Clanker: DeepSeek** via the `Agent` tool. The fixed model token is `ds`. Read-only calls use `subagent_type: "clanker:oc"`; write calls use the packaged `subagent_type: "clanker:supervisor"`.
...
- `--write` present -> `read_only: false`, a mandatory `worktree`, and `subagent_type: "clanker:supervisor"`. If the user did not name a branch, generate `clanker/deepseek-<short-timestamp>`. Absent `--write` -> `read_only: true` and `subagent_type: "clanker:oc"`.
...
- The task text must be: "Dispatch as Clanker: DeepSeek. lane=opencode. prompt=<task>. read_only=<bool>. worktree=<branch or omit>. model=ds. effort=<effort or omit>. Follow your relay/supervisor protocol."
