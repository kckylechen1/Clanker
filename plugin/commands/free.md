...
Dispatch the task below to **Clanker: Free** via the `Agent` tool. The fixed model token is `free`. Read-only calls use `subagent_type: "clanker:oc"`; write calls use `subagent_type: "clanker:writer"` without Sonnet supervision.
...
- `--write` present -> `read_only: false`, a mandatory `worktree`, and `subagent_type: "clanker:writer"`. If the user did not name a branch, generate `clanker/free-<short-timestamp>`. Absent `--write` -> `read_only: true` and `subagent_type: "clanker:oc"`.
...
- The task text must be: "Dispatch as Clanker: Free. lane=opencode. prompt=<task>. read_only=<bool>. worktree=<branch or omit>. model=free. effort=<effort or omit>. Follow your relay protocol."
