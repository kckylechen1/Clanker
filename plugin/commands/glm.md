...
Dispatch the task below to **Clanker: GLM** by invoking the `clanker:oc` subagent via the `Agent` tool (`subagent_type: "clanker:oc"`). The fixed model token is `glm`.
...
- The task text must be: "Dispatch as Clanker: GLM. prompt=<task>. read_only=<bool>. worktree=<branch or omit>. model=glm. effort=<effort or omit>. Follow your relay protocol."
