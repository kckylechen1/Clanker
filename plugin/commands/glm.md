---
description: "Dispatch a task to Clanker: GLM via Opencode ACP."
argument-hint: "[--background|--wait] [--write] [--effort <effort>] <task>"
allowed-tools: Agent
---

Dispatch the task below to **Clanker: GLM** via the `Agent` tool. The fixed model token is `glm`. Read-only calls use `subagent_type: "clanker:oc"`; write calls use the packaged `subagent_type: "clanker:supervisor"`.
...
- Fixed `model: "glm"`; do not ask the user for a model and do not infer another one.
- `--write` present -> `read_only: false`, a mandatory `worktree`, and `subagent_type: "clanker:supervisor"`. If the user did not name a branch, generate `clanker/glm-<short-timestamp>`. Absent `--write` -> `read_only: true` and `subagent_type: "clanker:oc"`.
...
- The task text must be: "Dispatch as Clanker: GLM. lane=opencode. prompt=<task>. read_only=<bool>. worktree=<branch or omit>. model=glm. effort=<effort or omit>. Follow your relay/supervisor protocol."
