...
Dispatch the task below to the **Clanker: Grok** via the `Agent` tool. Choose the Agent only after parsing the request: read-only calls use the mechanically read-only relay (`subagent_type: "clanker:grok"`); write calls use the packaged supervisor (`subagent_type: "clanker:supervisor"`). Both own the start-and-wait lifecycle and return the backend's final message + result fields.
...
- `--write` present → `read_only: false`, a mandatory `worktree`, and `subagent_type: "clanker:supervisor"` so the supervised write path owns correction/cancellation. If the user did not name a branch, generate a default `worktree` like `clanker/grok-<short-timestamp>`. Absent `--write` (or `--read-only` present) → `read_only: true` and `subagent_type: "clanker:grok"` (safe relay default; no worktree needed).
- `--model <model>` → `model` (passed through verbatim; grok maps it to `grok agent --model`). If omitted, pass `grok-4.5` explicitly; do not conflate it with the separate Composer 2.5 model.
...
- Launch exactly one `Agent` tool call with the selected subagent type (`"clanker:grok"` for read-only, `"clanker:supervisor"` for write) and `run_in_background: true`.
- The task text must be: "Dispatch as this Clanker. lane=grok. prompt=<task>. read_only=<bool>. worktree=<branch or omit>. model=<model or grok-4.5>. effort=<effort or omit>. Follow your relay/supervisor protocol."
- Do not call MCP dispatch tools in the main conversation.
- Do not wait for the subagent or relay a final result in this turn.
- After launching, tell the user: "Clanker: Grok started in the Claude Code background task list."
